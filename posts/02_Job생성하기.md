...
기존에는 `@EnableBatchProcessing`을 붙여줘야 했지만, Spring Boot 3.0 이상 및 Spring Batch 5.0 이상 환경에서는 `@EnableBatchProcessing`을 선언하지 않는 것이 기본 권장 사항입니다. (선언 시 자동 설정 기능이 비활성화되어 세밀한 커스텀 설정이 필요해집니다.)
...
- 최근 실패한 Job Parameter가 어떤 것들이 있고, 성공한 Job은 어떤 것들이 있는지
- 다시 실행한다면 어디서부터 시작하면 될지
- 어떤 Job에 어떤 Step들이 있었고, Step들 중 성공한 Step과 실패한 Step들은 어떤 것들이 있는지

등등 Batch 애플리케이션을 운영하기 위한 메타데이터가 여러 테이블에 나눠져 저장됩니다.

다음 코드를 yml 설정파일에 작성하면 테이블을 자동으로 생성해 줍니다.
